...
Math.minMax = function(a, b, c) { 
	return ( $.isNaN(b) )? a : ((b < a)? a : ((b > c)? c : b)); 
};
...
(function($) {
	...
	$.fn.logEvents = function( e ) {
		var events = (e || 'mousedown,mouseup,mouseover,mouseout,mousewheel,dragstart,click,blur,focus,load,unload,reset,submit,change,abort,cut,copy,paste,selection,drag,drop,orientationchange,touchstart,touchmove,touchend,touchcancel,gesturestart,gesturechange,gestureend').split(',');

		return this.each(function() {
			for (var i = 0; i < events.length; i++) {
				$(this).bind(events[i], function(e) {
					if (e.target.id !== 'log') 
	...
	$.fn.addScroll = function( settings ) {
		...
		return this.each(function() {
			var to, cont = $(this), wrap = $(this).parent(),
				...
				te, y0, tY, tT, tY1, speed, dist, min, max;
			...
			var matchCnt = function() { 
				cont.css({top: -Math.max(Math.round((shan.position().top - 2) * cheight / (sbar.height() - 4)), wheight - cheight)}); 
				setCtrl(); 
			};
			...
			var dragShStop = function(e) {
				$(document).unbind('mousemove', dragSh).unbind('mouseup', dragShStop);
				return false;
			};
			
			shan.bind('mousedown', function(e) { 
				$(this).data('my', Math.round(e.pageY) - $(this).position().top);
				$(document).bind('mousemove', dragSh).bind('mouseup', dragShStop);
				return false;
			});
			
			var getY = function(e) {
				var o = (e.type === 'touchmove' || e.type === 'touchend')? te.touches[0] : (e.touches? e.touches[0] : e); 
				return o.clientY; 
			};
			...
			ctrls.add(cont).bind('selectstart', function(e){ 
				return false; 
			});
			
			to = setTimeout(matchScr, 10);
			
			cont.attr('role', 'scroll').data('dragOn', false).bind('adjust', matchScr);
	...
	$.fn.scrollThumbs = function(settings) {
		...
		return this.each(function() {
			...
			var te, x0, tX, tT, tX1, speed, dist, min, scroll;
			...
			co.find('a').bind('setactive', function() {
			...
			if ( $.support.touch ) {
				...
				co.bind('mousedown', dragStart);
			}
			
			co.attr('role', 'scroll').data('dragOn', false);
			
			co.add(scleft).add(scright).bind('selectstart', function(e){ 
				e.preventDefault(); 
				return false; 
			});
		});
	...
	$.fn.swipe = function( leftFn, rightFn ) {
		...
		return this.each(function() {
			...
			var t = $(this);
			var dist, te, tx = 0, tx1 = 0, x0, tt, scroll, speed;
			...
			var dragStop = function(e) {
				...
				if ( $.support.touch ) {
					...
					$(document).unbind('mousemove', dragMove).unbind('mouseup click', dragStop);
				}
			...
			t.bind('dragcancel', function() {
				t.stop(true, false).animate({left: x0}, 200);
				return false;
			});
			
			t.bind('unswipe', function() {
				if ( $.support.touch ) {
					this.ontouchmove = null;
					this.ontouchend = null;
	...
	// Align an absolute positioned layer near another: top center or bottom center
	...
	$.fn.alignTo = function( el, settings ) {
		...
		return $(this).each( function() {
			var o = el.offset(),
				w = $(this).outerWidth(),
				h = $(this).outerHeight(),
				l = Math.round(o.left + (el.outerWidth() - w) / 2),
				t = o.top - settings.gap - h;
			
			if ( t < 0 ) {
				t = Math.min(o.top + el.outerHeight() + settings.gap, $(window).height() - h);
			}
	...
	$.fn.addHint = function(txt, settings) {
		...
		var hint, def = $('#' + settings.id), to, over = false;
		
		// Creating the popup container the first time
		if ( !def.length ) {
			def = $('<div>', { 'class': settings.id, id: settings.id }).hide().appendTo('body');
		}
		...
		var fade = function() {
			...
			hint.stop(true, false).fadeOut(200);
		};
			
		// Entering the trigger element
		var enter = function(e) {
			var el = $(e.target), tx = el.data('hint'), t, l, o;
			
			// Sometimes the event is triggered on the inner content
			if ( tx == null ) {
				el = el.parents([data=hint]).eq(0);
				if ( !el.length || !(tx = el.data('hint')))
					return true;
			}
			...
			// Inserting jQuery object
			if ( tx && tx.jquery ) {
				hint = tx.addClass( settings.id );
			} else {
				hint = def;
				hint.html( tx ).hide();
			}
			
			// Keeping the popup alive if mouse is over
			hint.hover(function() {
				to = clearTimeout(to);
				over = true;
				hint.stop(true, true).css('opacity', 1).show();
			}, function() {
				to = clearTimeout(to);
				over = false;
				fade();
			...
			hint.alignTo( el ).css('opacity', 1).fadeIn();
		...
		return this.each(function(i) {
			var t = txt || $(this).attr('title');
			if ( !t ) {
				return;
			}
			$(this).data('hint', t).removeAttr('title');
			$(this).bind($.support.touch? 'touchstart' : 'focus mouseenter', enter);
			$(this).bind('blur mouseleave', leave);
		});
	};
	...
	$.fn.shareIt = function( settings ) {
		...
		var im = encodeURIComponent( $('link[rel=img_src]').attr('content') );
		...
		return this.each(function() {
			...
			var e = $('<div>', { 'class': settings.id }).hide().appendTo('body');
			...
			if ( settings.googlePlus && gapi ) {
				e.append('<div class="likebtn" id="plusone"></div>');
	...
	$.fn.addPlayer = function( settings ) {
		...
		var embedHtml5 = function(t, f, w, h, p, a) {
			...
			el = $( (a? '<audio>' : '<video>'), {
				...
				preload: 'auto',
				poster: p,
				autoplay: settungs.auto
			}).appendTo(t);
			if ( $.isFunction(settings.complete) ) {
				el.bind('ended', settings.complete);
			}
			return el;
		};
		...
		var embedQt = function(t, f, w, h) {
			...
			el = $(html).appendTo(t);
			if ( $.isFunction(settings.complete) ) {
				el.bind('qt_ended', settings.complete);
			}
			return el;
		};
		...
		var embedWm = function(t, f, w, h) {
			...
			el = $(html).appendTo(t);
			if ( $.isFunction(settings.complete) ) {
				el.bind('playStateChange', function(e) {
	...
	$.fn.centerThis = function( settings ) {
		...
		var i, c, iw, ih, ww, wh, bw, r, l, t, rw, rh, ml, mr, mt, mb;
		...
		return this.each(function() {
			...
			if ( (bw = el.data('bw')) == null ) {
				el.data( 'bw', bw = parseInt( el.css('border-top-width') ) );
			}
			...
			ww = (c.width() || $('body').width()) - 2 * bw - ml - mr;
			wh = (c.height() || $('body').height()) - 2 * bw - mt - mb;

			if ( el[0].nodeName === 'IMG' && settings.fit ) {
				if ( ow > ww || oh > wh || settings.enlarge ) 
					r = Math.min(ww / ow, wh / oh);
				else 
					r = 1.0;
				ow = Math.round(ow * r);
				oh = Math.round(oh * r);
			...
			if ( settings.animate && settings.preScale && settings.preScale !== 1 ) {
				rw = ow * settings.preScale;
				rh = oh * settings.preScale;

				el.css({
					left: Math.round((ww - rw) / 2) + ml, 
					top: Math.round((wh - rh) / 2) + mt,
					width: rw,
					height: rh
				});
			}
			
			l = Math.round((ww - ow) / 2) + ml;
			t = Math.round((wh - oh) / 2) + mt;
			...
			if ( settings.animate ) {
				el.animate( {
					...
					width: ow,
					height: oh
				}, settings.speed, settings.effect, settings.complete );
			} else {
				el.css( {
					top: t, 
					left: l, 
		...
		return this.each(function() {
			...
			if ( settings.gateway === 'paypal' ) {
				...
				if ( settings.shipping != null ) {
					fs.addInput(id.shipprice, parseInt(settings.shipping) || 0, 'hidden');
				}
				if ( settings.handling != null ) {
					fs.addInput(id.handling, parseInt(settings.handling) || 0, 'hidden');
				}
	...
	$.fn.setupMap = function( settings ) {
		...
		return this.each(function() {
			...
			t.data('fresh', true).bind('adjust', adjust);
	...
	*/
	...
	$.fn.turtleGallery = function( settings, text, id ) {
		...
		if ( !settings.licensee && location.protocol.startsWith('http') && !cookie('ls') ) {
			setTimeout(function() {
				$('<div class="skinlogo"><h3>Turtle skin</h3><p>Unlicensed</p></div>').popupBox();
				cookie('ls', true);
			}, 1000); 
		}
		...
		$.fn.centerThis.defaults.animate = settings.fitAnimate;
		...
		return this.each( function() {
			...
			var fadeCtrl = function() {
				if ( cmo ) { 
					cto = setTimeout(function(){ fadeCtrl() }, 1000);
				} else {
			...
			var showCaption = function() {
				...
				bottom.animate({bottom: 0}, 500, function() {
			...
			var showImg = function( n ) {	
				...
				if ( cimg ) {
					...
					pimg = cimg;
					pimg.css( {zIndex: 0} );
					pimg.find('.' + id.main).trigger('unswipe').unbind('touchstart');
					pimg.unmousewheel();
				}
			...
			var imgReady = function( o ) {
				...
				wait.stop(true, false).fadeOut(100);
				
				if ( pimg ) {
					var p = pimg;
					pimg.stop(true, false).fadeOut(settings.speed / 2, function() { 
						p.remove();
						removePlayer(p);
					});
					pimg = null;
				...
				cimg.append(o);
				cimg.css('opacity', 0).centerThis( {
					preScale: settings.preScale,
					fit: settings.fitImage,
					marginTop: scrollboxHeight()
				...
				if ( settings.hash === 'number' ) {
					...
					var s = images.eq(curr).attr('href');
					if ( s ) {
						$.history.load(s.substr(s.lastIndexOf('/') + 1));
					}
				}
			};
			...
			var createInfo = function(im, n) {
						...
						a.click( function() {
							...
							if ( t === id.map ) {
								e.slideToggle('fast', function() {
									$(this).children('.' + id.mapcont).trigger('adjust'); 
								});
							} else {
								e.slideToggle('fast');
							}
				...
				if ( d = im.data(id.link) ) {
					...
					a.addHint( im.data(id.isoriginal)? text.original : text.hiRes );
				}
						...
						if ( t === id.map ) {
							...
							if ( settings.mapAll ) {
								mc.setupMap({
									...
									markers: markers,
									curr: parseInt(im.data('mapid'))
								});
							} else {
								mc.setupMap({
									map: d,
									label: im.data(id.caption).stripHTML()
								});
							}
						} else if ( t === id.shop ) {
			...
			var setupHeader = function() {
				...
				e.find('.' + id.help + '>a').click( function() {
					showHelp( helpWindow );
				});
			};
			...
			var setupThumbs = function( t ) {
				var t, a, i, im, l, w = 0;
					...
					nl = '<span class="' + id.newItem + '">' + text.newItem + '</span>',
				...
				images.each( function(n) {
					...
					if ( im.attr('src').endsWith('/' + settings.loadImg) ) {
						l = l.replace(re, settings.thumbs + '/');
						im.add(i).attr('src', l);
					}
					else {
					...
					if ( settings.markNewDays && (d - parseInt(im.data(id.modified) || 0)) <= settings.markNewDays ) {
						t.add(a).append( nl );
					}
	...
	$.fn.turtleGallery.defaults = {
		...
		preScale: 0.96,				// size of the image before the transitions starts
		...
		newDays: 30,				// : days passed by considered a picture is 'new'
		...
		fitAnimate: true,			// Animate the fitting?
		...
		videoHeight: 480,
		controlbarHeight: 24,
		help: '<h2>{0}</h2><ul class="help index"><li><span>1</span>{1}</li><li><span>2</span>{2}</li><li><span>3</span>{3}</li></ul><hr><ul class="help gall"><li><span>1</span>{4}</li><li><span>2</span>{5}</li><li><span>3</span>{6}</li><li><span>4</span>{7}</li><li><span>5</span>{8}</li><li><span>6</span>{9}</li><li><span>7</span>{10}</li></ul><p>{11}</p>',
		helpgall: '<h2>{0}</h2><ul class="help gall"><li><span>1</span>{4}</li><li><span>2</span>{2}</li><li><span>3</span>{6}</li><li><span>4</span>{7}</li><li><span>5</span>{8}</li><li><span>6</span>{9}</li><li><span>7</span>{10}</li></ul><p>{11}</p>'
	};
